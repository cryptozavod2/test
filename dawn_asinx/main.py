...
class DawnValidatorBot:
    ...
    async def fetch_points(self, headers: Dict[str, str]) -> int:
        ...
        for i in range(10):
            try:
                ...
                self.log_colored("ERROR", f"Failed to fetch points: {str(e)}", Colors.WARNING)
                ex = e
                await asyncio.sleep(.5)
        self.log_colored("ERROR", f"Failed to fetch points: {str(ex)}", Colors.ERROR)
        return 0
    ...
    async def verify_social_media(self, account: Dict[str, str], proxy: Optional[str] = None, points: Optional[int] = 0) -> None:
        email = account['email']
        ...
        social_types = []
        if points < 5000:
            social_types.append("twitter_x_id")
        if points < 10000:
            social_types.append("discordid")
        if points < 15000:
            social_types.append("telegramid")
...
""")
    ...
    async def process_account(self, account: Dict[str, str]) -> int:
        ...
        if points < 15000:
            await self.verify_social_media(account, proxy, points)
    ...
    async def countdown(seconds: int) -> None:
        print(f"\r[{datetime.now().strftime('%H:%M:%S')}] {Colors.WARNING}Waiting: {seconds}s\n{Colors.RESET}", end='')
...
message_queue = Queue()
bot_token = "6760894396:AAGGJ4dReczYrT-OojvAw1IR56eiD5DOt6A"
telegram_chat_id = "971073478"
    ...
    try:
        ...
        while True:
            ...
            for i, points in enumerate(points_array):
                ...
                list_resp.append([accounts[i]['email'],total_points])
